...
function getDateDetails(date) {
  ...
  else return { year: date.getFullYear(), month: date.getMonth(), day: date.getDate()} 
...
function nextNewYear() {
  ...
  let diff = nextYear.getTime() - newDate.getTime()  
...
function addDays(date, days) {
  ...
  let addDay = date.setDate(date.getDate() + days)
...
// Якщо date не є об'єктом Date, повертаємо рядок
  ...
  let dayOfWeek = daysOfWeek[dayNum]
  // Повертаємо об'єкт з початковою датою та днем тижня.
  return {inputDate: isoDate, dayOfWeek: dayOfWeek}
}
...
function getDaysInMonth(date) {
  ...
  let daysMonth = nextMonth.getDate()
  // Повертаємо об'єкт з початковою датою та кількістю днів у місяці.
  return { inputDate: date, daysInMonth: daysMonth }
}
...
function getFormattedTime(date) {
  ...
  let formatTime = `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`
  // Повертаємо об'єкт з початковою датою та отформатованим часом.
  return { inputDate: isoDate, formatedTime: formatTime }
}
...
function getAge(birthDate) {
  ...
  return { birthDate: birthDate, age}
}
...
function compareDates(date1, date2) {
  ...
  return { date1, date2, comparison: comparison}
}
...
function getDaysDifference(startDate, endDate) {
  ...
  let comparison = miliStartDate - miliEndDate
  // Перетворення різниці в мілісекундах у дні поділивши мілісекунди на (1000 * 60 * 60 * 24).
  let daysDifference = comparison / (1000 * 60 * 24)
  // Збереження дат для виведення в форматі ISO.
